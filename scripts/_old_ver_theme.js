...
"use strict";
  ...
  $(".header div.menu div.container > ul > li").each(function(index, element) {
    if(index<menumiddle){ leftwidth+=$(this).width(); }
  else { rightwidth+=$(this).width(); }
  if(index==menumiddle){ $(this).addClass('right-side'); }
  });
  if(leftwidth>rightwidth){ $(".header div.menu div.container > ul > li:last").css('padding-right', (leftwidth-rightwidth)+'px'); } else { $(".header div.menu div.container > ul > li:first").css('padding-left', (rightwidth-leftwidth)+'px'); }
